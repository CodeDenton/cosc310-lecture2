...
class Cart:
    ...
    def add_item(self, item: dict, qty: int = 1) -> None:
        ...
        if qty < 1:
            raise ValueError("Can't add qty less than 1 to cart")
        if not item["available"]:
            raise OutOfStockError("Item is not available to add to cart")
        for i in self.lines:
            if(i["item_id"] == item["id"]):
                i["qty"] += qty
                return
        new_item = {"item_id": item["id"], "name": item["name"], "price": item["price"], "qty": qty}
    ...
    def remove_item(self, item_id: int) -> None:
        ...
        for i in self.lines:
                    if(i["item_id"] == item_id):
                        self.lines.remove(i)
                        return
        raise KeyError("Cannot remove item that is not in cart")
